...
int worker_fun(){
	...
	if((skey = ftok("./mrs", 1001)) == -1) {  
        //fprintf(stderr,"[pid:%u]Creat Key Error：%s\n", getpid(), strerror(errno));  
		logmsg(log_error, "worker Creat Key Error：%s\n", strerror(errno));
        goto failed; 
    }
...
restart:
	...
	if((rkey = ftok("./mrs", 1001)) == -1) {  
        //fprintf(stderr,"Creat rKey Error：%s\n",strerror(errno));  
		logmsg(log_error, "master Creat rKey Error：%s\n",strerror(errno));
        exit((int)-1);  
    }
